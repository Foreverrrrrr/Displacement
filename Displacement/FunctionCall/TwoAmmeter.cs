...
namespace Displacement.FunctionCall
{
    public class TwoAmmeter
    {
        ...
        {
            get { return _twoa; }
            set { _twoa = value; MainWindowViewModel.thisModel.TwoElectric = value; }
        }
        ...
        public static TwoAmmeter thiselectricity { get; set; }

        public TwoAmmeter(string com, int baudrate)
        {
        ...
        private void ReadPower()
        {
            while (true)
            {
                ...
                if (read.IsSuccess)
                {
                    ...
                    if (t >= 0)
                        TwoA = busRtuClient.ByteTransform.TransSingle(read.Content, 0);
                    if (!IsOpen)
                        IsOpen = true;
                }
                else
