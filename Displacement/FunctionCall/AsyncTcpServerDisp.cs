...
namespace Displacement.FunctionCall
{
    ...
    /// 位移
    /// </summary>
    public class AsyncTcpServerDisp : ClientSession
    {
        ...
        /// 是否接收数据
        /// </summary>
        public static bool IF_Read { get; set; } = true;
        public AsyncTcpServerDisp(string ip, int port)
        {
            ...
            if (ar.AsyncState is ClientSession client)
            {
                ...
                try
                {
                    ...
                    if (IF_Read)
                    {
                        ...
                        if (CRC(data))
                        {
                            byte[] datasize = new byte[1200];
                            Array.Copy(data, 10, datasize, 0, 1200);
                            //msg = Encoding.ASCII.GetString(data, 0, length); //接收数据
                            OnTCPReadEvent?.Invoke(DateTime.Now, new IPEndPoint(client.EndPoint.Address, client.EndPoint.Port), datasize);
        ...
        public void AsyncWrite(byte[] meg)
        {
            if (IsCommet)
            {
                var emp = sockets[0];
        ...
        public void AsyncWrite(string ip, int port, byte[] meg)
        {
            if (IsCommet)
            {
                var emp = sockets.Find(e => e.EndPoint.Address.ToString() == ip && e.EndPoint.Port == port);
