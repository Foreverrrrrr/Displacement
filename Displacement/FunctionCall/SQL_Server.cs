...
using static Displacement.AutomaticMain.Auto;
...
namespace Displacement.FunctionCall
{
    public class SQL_Server
    {
        ...
        /// 数据库连接字符串
        /// </summary>
        public static readonly string ConnectionString_Default = ConfigurationManager.ConnectionStrings["Displacement.Properties.Settings.ConnectionString"].ConnectionString;

        private static SqlConnection sql_con;//声明一个SqlConnection
        ...
        public static DataSet SpecialQuery(DateTime TimeMax, DateTime TimeMin)
        {
            ...
            return SQL_Server.ExecuteDataSet(CommandType.Text, $"SELECT * FROM Test WHERE time BETWEEN '{TimeMax.ToString("yyyy-MM-dd HH:mm:ss.sss")}' AND '{TimeMin.ToString("yyyy-MM-dd HH:mm:ss.sss")}'");
        ...
        public static DataSet SpecialQuery(DateTime TimeMax, DateTime TimeMin, bool res)
        {
            ...
            if (res)
                ...
                sql = "NG";
            return SQL_Server.ExecuteDataSet(CommandType.Text, $"SELECT * FROM Test WHERE time BETWEEN '{TimeMax.ToString("yyyy-MM-dd HH:mm:ss.sss")}' AND '{TimeMin.ToString("yyyy-MM-dd HH:mm:ss.sss")}' AND [total_result]  = '{sql}'");

        }

        public static void Write(Sql_SeverTest floats)
        {

            SQL_Server.ExecteNonQuery(CommandType.Text, $"insert into Test values('{floats.Time.ToString("yyyy-MM-dd HH:mm:ss.sss")}'" +
                $",{floats.total_production},{floats.NG_number},{floats.yield},{floats.ct_s},{floats.utilization},{floats.total_running_time},{floats.normal_duration},{floats.bnormal_duration}" +
                $",'{floats.carrier_number}','{floats.total_result}','{floats.one_result}','{floats.two_result}','{floats.three_result}'" +
                $",'{floats.four_result}',{floats.bunch_movement_value},{floats.four_standard} "+
                $",{floats.push_movement_upper_limit},{floats.push_movement_value},{floats.push_movement_lower_limit}" +
                $",{floats.pull_movement_upper_limit},{floats.pull_movement_value},{floats.pull_movement_lower_limit}" +
                $",{floats.push_force_upper_limit},{floats.push_force_value},{floats.push_force_lower_limit}" +
                $",{floats.pull_force_upper_limit},{floats.pull_force_value},{floats.pull_force_lower_limit}" +
                $",'{floats.five_result}',{floats.no_load_current_upper_limit},{floats.no_load_current},{floats.no_load_current_lower_limit},{floats.axis_jump_range},{floats.axis_jump_value},{floats.worm_jump_range},{floats.worm_jump_value},{floats.voltage}" +
                $",'{floats.six_result}',{floats.greasing_value},{floats.greasing_speed},'{floats.seven_result}')");
